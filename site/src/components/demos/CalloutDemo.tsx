/**
 * CalloutDemo: the live, interactive Nockerl callout (info block) island for web.
 *
 * A callout is a STATIC, persistent editorial emphasis block embedded IN body
 * content, one of note / tip / important / warning / caution / quote. It is NOT a
 * banner: it has no dismiss, no action button, no app-state semantics. It reads as
 * part of the prose (a recessed aside), where the banner is a lifted transient
 * alert. The two SHARE the family signature (a filled status ICON DISC), so the
 * vocabulary is one; the callout just sinks into the page and carries rich prose.
 *
 * There is NO shipped Callout on any platform yet (no `NockerlCallout` /
 * `NockerlAside` in Android Compose, none in Voice/Swift). It is designed
 * ORIGINALLY from the design laws + the Nockerl emphasis vocabulary that IS
 * shipped:
 *   • the agent-transcript ThinkingSegment: italic + muted reasoning prose, an
 *     in-content aside, NOT a card (chat/ui/AgentTranscriptPanel.kt) → the
 *     `quote` variant.
 *   • the agent-transcript ToolSegment: a status-tinted inset embedded in the
 *     message flow (chat/ui/AgentTranscriptPanel.kt) → the recessed surface.
 *   • the markdown body styling: the body prose, links, and the quiet inline
 *     `code` tint a callout's rich content reuses (chat/ui/MarkdownContent.kt).
 *   • the Voice SettingsCard eyebrow: an UPPERCASE, tracked, muted label as the
 *     section lead (NockerlVoice/UI/SettingsComponents.swift) → the title row.
 *
 * TWO TREATMENTS:
 *   • NORMAL tones (note / tip / warning / caution / notice / quote): the family DISC,
 *     a filled status circle with a knocked-out glyph, on a SOLID recessed surface.
 *     The color lives in the disc, never a left rail.
 *   • the EMPHASISED `important` tone ONLY: NESTED HAIRLINE FRAMES. Concentric
 *     rounded borders step inward with FALLING opacity (a box-within-a-box), with
 *     small inset gaps between them, for real dimensional depth. This is the
 *     reserved "this is the thing you must not miss" treatment.
 *
 * The new transparency law, verbatim: surfaces are SOLID + dimensional. The
 * callout is `--color-canvas-alt` (a recessed well where content sinks) with a NEUTRAL
 * inner top shadow, NO glow, NO colored wash. Transparency is for STATE only; the
 * nested frames step opacity via color-mix off ONE `--co-c` custom property; tints
 * come from the status tokens. Status color for warm tones only; cyan is the ONE
 * editorial-emphasis tone (`important`); note/quote use on-card, never cyan. Never
 * color alone: every callout pairs the disc (or italic) + text. 12px panel radius.
 * Flash-free (disc + surface static; a link animates only color/underline). The
 * disc is centred to the first line; any link is keyboard-focusable with a ring.
 *
 ...
 * geometry (icon viewBox, transition curve).
 */
import { useState } from 'react';
import { ALERT_INTENT, NockerlIcon, NockerlStatusDisc, type AlertIntent, type ComposeContract } from '@dizyx/nockerl-react';

export type CalloutTone = 'note' | 'tip' | 'important' | 'warning' | 'caution' | 'notice' | 'quote';
...
const DISC_INTENT: Record<Exclude<CalloutTone, 'note' | 'quote'>, AlertIntent> = {
  tip: 'success',
...
const TONE_LABEL_COLORED: Record<CalloutTone, boolean> = {
  note: false,
  tip: true,
  important: true,
  warning: true,
...
// ─── Knockout glyphs (FILLED: they cut a dark stencil out of the colored disc) ──
// currentColor inherits the disc's knockout ink (--color-canvas). The note / tip /
// caution marks echo the banner family; important + warning carry editorial marks.
const ICONS: Record<Exclude<CalloutTone, 'quote'>, React.ReactNode> = {
 ...
 */
export function Callout({ children, tone = 'note', title, icon = true }: CalloutProps) {
  ...
  return (
    <div className="nk-co-demo">
      ...
      <div className="nk-co-demo__group">
        ...
        <Callout tone="tip" title="Pro tip" icon={false}>
          {
      ...
      >
        ...
        <Callout tone="important" title="Try it">
          {
