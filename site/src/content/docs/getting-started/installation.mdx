...
1. **Point the `@dizyx` scope at GitHub Packages.** Add an `.npmrc` to your project (or
   `~/.npmrc`):
   ...
   `GITHUB_TOKEN` is read from the environment, a classic PAT (or fine-grained token)
   with `read:packages`. **Never commit the token**; keep it in the environment or a
   secret store. In CI, the Actions `GITHUB_TOKEN` already has `packages: read`.
...
<DocTabs surface={false}>
  ...
  **Status: available.** Two Maven artifacts on **GitHub Packages**: `design-tokens` (the
  ...
  `read:packages` PAT **even to read**, and Gradle cannot route through the credential store proxy, so
