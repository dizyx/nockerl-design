...
the next bump. All three platforms ship from **one version line** (currently `2.1.0`): the
...
4. **Load the brand fonts.** The tokens name **Outfit** (`--font-family-sans`) and **Space
...
5. **Use a component.** Each component injects its own scoped `<style>`, so there is **no
...
source. **Both are available now** at the same `2.1.0` version line: tokens *and*
components.
